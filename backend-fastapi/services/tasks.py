...
def match_data(league: str):
    ...
    for match in response:
        ...
        seriesHandler(new, tournament_name=match["Name"], split_name=match["Split"], overview_page=match["OverviewPage"])
...
def seriesHandler(match: str, tournament_name: str, split_name: str, overview_page: str):
    existing = session.query(Series).filter_by(SeriesKey=match.SeriesKey).one_or_none()
    if existing:
        return existing

    split = splitHandler(split_name=split_name, tournament_name=tournament_name, date=match.Date, overview_page=overview_page)
    new = Series(
...
def splitHandler(split_name: str, tournament_name: str, date: datetime, overview_page: str):
    if not split_name:
        return None
...
def handleStandings(league: str, overview_page):
    response = site.query(
        tables=["Standings"],
        fields=[
