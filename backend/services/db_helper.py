...
class Query():
    ...
    def get_split(self, split: str, year: int, isInternational: bool):
        clause = f"{split} {year}"
        statement = select(Split).where(Split.Name.like(f"{clause}")) if isInternational else select(Split).where(Split.Name.like(f"{clause} %"))
        result = self.session.scalars(statement).all()
        if not result:
